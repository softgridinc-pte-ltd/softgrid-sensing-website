...
const engines: Engine[] = [
  {
    icon: <Radio className="w-6 h-6 text-primary-500" />,
    name: 'Connect',
    ...
    <section className="bg-navy-900 py-16 md:py-24 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        ...
        <div data-animate className="hidden md:flex items-center justify-center gap-2 mb-6">
          {engines.map((engine, i) => (
            <div key={engine.name} className="contents">
              ...
              {i < engines.length - 1 && (
                <ChevronRight className="w-4 h-4 text-primary-500/50 flex-shrink-0" />
              )}
            </div>
          ))}
        </div>
        ...
        <div data-animate className="flex justify-center mb-12 md:mb-16">
          <div className="inline-flex items-center gap-2 bg-primary-500/10 border border-primary-500/20 rounded-full px-4 py-1.5">
            <div className="w-1.5 h-1.5 bg-primary-500 rounded-full animate-pulse-slow" />
            <span className="text-primary-400 text-sm font-medium">Closed-loop: outcomes feed back into Intelligence</span>
          </div>
        </div>
        ...
        <div data-animate-stagger className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {engines.map((engine) => (
            <div
              data-animate
              key={engine.name}
              className="bg-navy-800 border border-navy-700 rounded-xl p-6 md:p-8 hover:border-primary-500/30 transition-all duration-300"
            >
              {/* Engine header */}
              <div className="flex items-center gap-3 mb-4">
                <div className="w-11 h-11 bg-primary-500/10 border border-primary-500/20 rounded-lg flex items-center justify-center flex-shrink-0">
                  {engine.icon}
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-white">{engine.name} Engine</h3>
                  <p className="text-sm text-primary-400">{engine.subtitle}</p>
                </div>
              </div>
